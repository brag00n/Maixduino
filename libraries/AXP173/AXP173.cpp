...
void AXP173::begin(bool isInited)
{
    ...
    // // Write1Byte(0x30, 0x80); //Disable vbus hold limit
...
void AXP173::ScreenBreath(uint8_t brightness)
{
    if (brightness > 12) 
    {
        brightness = 12;
...
// Input Power Status 
uint8_t AXP173::GetInputPowerStatus()
{
    return Read8bit(0x00);
}

// Battery Charging Status 
uint8_t AXP173::GetBatteryChargingStatus()
{
...
//------------------------------------------
void  AXP173::EnableCoulombcounter(void)
{
    Write1Byte( 0xB8 , 0x80 );
}

void  AXP173::DisableCoulombcounter(void)
{
    Write1Byte( 0xB8 , 0x00 );
}

void  AXP173::StopCoulombcounter(void)
{
    Write1Byte( 0xB8 , 0xC0 );
}

void  AXP173::ClearCoulombcounter(void)
{
    Write1Byte( 0xB8, Read8bit(0xB8) | 0x20);    // Only set the Clear Flag
}

uint32_t AXP173::GetCoulombchargeData(void)
{
    return Read32bit(0xB0);
}

uint32_t AXP173::GetCoulombdischargeData(void)
{
    return Read32bit(0xB4);
}

float AXP173::GetCoulombData(void)
{
...
uint8_t AXP173::GetWarningLeve(void)
{
    Wire.beginTransmission(AXP_ADDR);
    Wire.write(0x47);
    Wire.endTransmission();
...
// Note: now AXP173 have the Shutdown Voltage of 3.0V (B100) Def in REG 31H
uint8_t AXP173::GetWarningLevel(void)
{
...
float AXP173::GetBatChargeCurrent()
{
    float ADCLSB = 0.5;
...
float AXP173::GetBatCoulombInput()
{
    uint32_t ReData = Read32bit( 0xB0 );
    return ReData * 65536 * 0.5 / 3600 /25.0;
}

float AXP173::GetBatCoulombOut()
{
...
void AXP173::SetLDO2( bool State )
{
...
void AXP173::SetLDO3(bool State)
{
    uint8_t buf = Read8bit(0x12);
    if( State == true )
    {
...
void AXP173::SetChargeCurrent(uint8_t current)
{
...
void AXP173::SetAdcState(bool state)
{
    Write1Byte(0x82, state ? 0xff : 0x00);  // Enable / Disable all ADCs
}

// AXP173 have a 6 byte storage, when the power is still valid, the data will not be lost
void AXP173::Read6BytesStorage( uint8_t *bufPtr )
{
...
// AXP173 have a 6 byte storage, when the power is still valid, the data will not be lost
void AXP173::Write6BytesStorage( uint8_t *bufPtr )
{
    // Address from 0x06 - 0x0B
    Wire.beginTransmission(AXP_ADDR);
    Wire.write(0x06);
